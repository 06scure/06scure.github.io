...
在搭建多进程的框架的时候，初始化进程需要传递一些参数。本项目在参阅[blakeblackshear/frigate: NVR with realtime local object detection for IP cameras](https://github.com/blakeblackshear/frigate)后决定将所有的数据交给`Pydantic`管理。而我们的后端框架`fastapi`，也是用`pydantic`管理的，模块复用了。
